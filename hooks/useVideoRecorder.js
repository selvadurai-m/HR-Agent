...
export function useVideoRecorder() {
  ...
  const startRecording = useCallback(async (stream) => {
    ...
    try {
      ...
      logger.log('✅ Video recording started:', selectedMimeType);
      return true;
    } catch (err) {
    ...
    return new Promise((resolve) => {
      ...
      mediaRecorderRef.current.onstop = () => {
        try {
          ...
          logger.error('Error creating blob:', err);
          setRecordingError(err.message);
          resolve(null);
        } finally {
          chunksRef.current = [];
  ...
  const uploadRecording = useCallback(async (videoBlob, interviewId) => {
    if (!videoBlob || !interviewId) {
      logger.error('Missing videoBlob or interviewId');
      return null;
    }
    ...
    try {
      ...
      logger.log('📤 Uploading recording:', filePath);
